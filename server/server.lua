...
local function getRegionalWeatherSlots(regionName)
    if not useRegionalWeather then
        return {}
    end
    
    return exports['weathersync']:getWeatherSlots(regionName) or {}
end
...
AddEventHandler("weathersync:requestRegionalWeather", function(regionName)
    if useRegionalWeather then
        local slots = getRegionalWeatherSlots(regionName)
    ...
    while true do
        ...
        if not weatherIsFrozen then
            if weatherTicks >= weatherInterval then
                ...
                if useRegionalWeather then
                    exports['weathersync']:advanceWeatherTick()
                    exports['weathersync']:saveCache()
                end
