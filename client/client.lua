...
RegisterNetEvent("weathersync:receiveRegionForecast")
RegisterNetEvent("weathersync:receiveAllForecasts")
...
AddEventHandler("weathersync:applyRegionalWeather", function(weatherData)
	if not syncEnabled then
		return
	end

	if not weatherData or not weatherData.variant then
		return
	end
	...
	if Config.isRDR then
		...
		if Config.dynamicSnow then
			if Config.dynamicSnowTypes[translatedWeather] then
				snowOnGround = true
				if Config.isRDR then
					SetSnowCoverageType(3)
				end
		...
		snowOnGround = Config.dynamicSnow and isSnowyWeather(translatedWeather)
	end
...
AddEventHandler("weathersync:receiveAllForecasts", function(allForecasts)
	if not allForecasts then return end
	
	local regionGroups = ConfigRegionWeather and ConfigRegionWeather.RegionGroups or {}
	local regions = ConfigRegionWeather and ConfigRegionWeather.Regions or {}
	
	print("^3________________________________________________________________^7")
	print("^3              WEATHER FORECAST - ALL REGIONS                   ^7")
	print("^3________________________________________________________________^7")
	
	for groupName, groupRegions in pairs(regionGroups) do
		print("^6" .. groupName .. "^7")
		for _, regionName in ipairs(groupRegions) do
			local regionData = regions[regionName]
			local forecast = allForecasts[regionName]
			if regionData and forecast and #forecast > 0 then
				local weatherStr = "  ^3" .. regionData.Name .. "^7 > "
				for i = 1, #forecast do
					weatherStr = weatherStr .. forecast[i].weather
					if i < #forecast then
						weatherStr = weatherStr .. " > "
					end
				end
				print(weatherStr)
			elseif regionData then
				print("  ^3" .. regionData.Name .. "^7 (no data)")
			end
		end
		print("")
	end
...
AddEventHandler("weathersync:updateAdminUi", function(time, timescale, syncDelay)
...
Citizen.CreateThread(function()
	...
	RegisterCommand("forecast", function(source, args, raw)
		TriggerEvent("weathersync:toggleForecast")
	end, false)

	RegisterCommand("allforecast", function(source, args, raw)
		TriggerServerEvent("weathersync:requestAllForecasts")
	end, false)
end)
